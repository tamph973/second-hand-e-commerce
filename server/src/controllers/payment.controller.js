...
export const handleVNPayReturn = async (req, res, next) => {
	try {
		...
		if (payment.vnp_ResponseCode === '00') {
			res.redirect(`${env.FRONTEND_URL}/checkout/success/?orderId=${payment.orderId}`);
		}
	} catch (error) {
		next(error);
	}
...
export const handleMoMoReturn = async (req, res, next) => {
	try {
		const data = await PaymentService.handleMoMoReturn(
			req.query,
			req.body.paymentId,
		);
		...
		if (data.resultCode === '0') {
			res.redirect(
				`${env.FRONTEND_URL}/checkout/success/?orderId=${data.orderId}`,
			);
		}
	} catch (error) {
		next(error);
